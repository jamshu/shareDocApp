// One-time (idempotent, re-runnable) Odoo provisioning for shareDocApp.
// Creates the manual models/fields the app expects (same x_studio_* names the
// old Studio instance had, so app code needs zero changes), access rights,
// the notes company record rule, and puts existing users in the Documents group.
// Also empirically tests which upload path works for documents.document.
//
...
async function main() {
	...
	for (const f of [
		...
		{ name: 'x_studio_editor_mode', ttype: 'char' },
		{ name: 'x_studio_date', ttype: 'date' },
		{ name: 'x_studio_permission', ttype: 'char' },
	...
	/* ---- documents user group for existing users ---- */
	console.log('Documents group:');
	const docGroups = await x('ir.model.data', 'search_read', [
		[['module', '=', 'documents'], ['name', 'like', 'group_documents'], ['model', '=', 'res.groups']],
		['name', 'res_id']
	]);
	console.log('  found:', docGroups.map((g) => `${g.name}=${g.res_id}`).join(', ') || 'none');
	const docUserGroup = docGroups.find((g) => g.name === 'group_documents_user') || docGroups[0];
	if (docUserGroup) {
		const users = await x('res.users', 'search', [[['share', '=', false], ['active', '=', true]]]);
		await x('res.users', 'write', [users, { group_ids: [[4, docUserGroup.res_id]] }]).catch(() =>
			// Odoo <19 field name
			x('res.users', 'write', [users, { groups_id: [[4, docUserGroup.res_id]] }])
		);
		console.log(`  users ${users.join(',')} added to group ${docUserGroup.name}`);
	}
	...
	/* ---- upload path test: does datas-on-create work for documents.document? ---- */
	console.log('Upload path test:');
	const [anyFolder] = await x('documents.document', 'search', [[['type', '=', 'folder']]], { limit: 1 });
	const b64 = Buffer.from('setup-test hello').toString('base64');
	let path = null;
	const docId = await x('documents.document', 'create', [
		{ name: '__setup_test.txt', folder_id: anyFolder, datas: b64, mimetype: 'text/plain' }
	]);
	const [doc] = await x('documents.document', 'read', [[docId], ['file_size', 'datas']]);
	if (doc.file_size > 0 && doc.datas) path = 'datas';
	await x('documents.document', 'unlink', [[docId]]);
	if (!path) {
		const attId = await x('ir.attachment', 'create', [
			{ name: '__setup_test.txt', mimetype: 'text/plain', raw: 'setup-test hello' }
		]);
		const docId2 = await x('documents.document', 'create', [
			{ name: '__setup_test.txt', folder_id: anyFolder, attachment_id: attId }
		]);
		const [doc2] = await x('documents.document', 'read', [[docId2], ['file_size', 'datas']]);
		if (doc2.file_size > 0 && doc2.datas) path = 'attachment-first';
		await x('documents.document', 'unlink', [[docId2]]);
	}
	console.log(`  WORKING UPLOAD PATH: ${path || 'NONE — investigate manually'}`);
