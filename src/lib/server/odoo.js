...
async function createUser({ name, email, password }) {
	...
	try {
		// Odoo 19: res.users group field is group_ids; xmlid_to_res_id is gone,
		// resolve xml ids via ir.model.data instead. Internal user + Documents user.
		const rows = await adminExecute('ir.model.data', 'search_read', [
			['|',
				'&', ['module', '=', 'base'], ['name', '=', 'group_user'],
				'&', ['module', '=', 'documents'], ['name', '=', 'group_documents_user']
			],
			['res_id']
		]);
		const gids = rows.map((r) => r.res_id);
		if (gids.length) vals.group_ids = [[6, 0, gids]];
	} catch {
...
export async function deleteUserAccount(uid) {
	...
	const acts = await adminExecute('mail.activity', 'search', [[['user_id', '=', uid]]], {
		context: { active_test: false } // done reminders are archived, not deleted
	});
