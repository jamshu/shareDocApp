...
/**
 * Documents vault gate. Odoo Documents folders are shared instance-wide, so a
 * stranger who signs up their own org must not reach them: only users whose
 * company is in DOCS_ALLOWED_COMPANY_IDS (default "1") may use /api/documents*.
 */
export async function requireDocsUser(cookies) {
	const u = await requireApprovedUser(cookies);
	const allowed = (env.DOCS_ALLOWED_COMPANY_IDS || '1').split(',').map(Number);
	if (!allowed.includes(u.ctx.allowed_company_ids?.[0])) {
